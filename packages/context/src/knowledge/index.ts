...
import {
  ...
  type DiscoveredKnowledgeFile,
  type IndexedKnowledgeChunk,
  type IndexedKnowledgeFile,
  type KnowledgeChunk,
...
async function readUtf8(
  ...
  try {
    ...
    const relative = path.relative(file.root, opened);
    if (
      !info.isFile() ||
      opened !== file.path ||
      relative === ".." ||
      ...
      throw new Error(`Knowledge file changed after discovery: ${file.path}`);
    if (info.size > maximumBytes)
      throw new Error(`File exceeds byte limit: ${file.path}`);
    ...
    if (offset !== info.size)
      throw new Error(`Knowledge file changed while reading: ${file.path}`);
...
export async function parseKnowledgeFile(
  ...
  return chunkKnowledge(await readUtf8(file, maximumBytes), file.path);
}
...
function mapChunk(row: any): IndexedKnowledgeChunk {
  return {
    ...
    charOffset: Number(row.char_offset),
  };
}
...
CREATE INDEX IF NOT EXISTS chunks_file ON chunks(file_id,ordinal);`);
      ...
      if (version && version.version !== KNOWLEDGE_SCHEMA_VERSION)
        throw new Error(
          `Unsupported knowledge schema version ${version.version}`
        );
      this.db
        .prepare(
  ...
  private initializeFts(): void {
    immediateTransaction(this.db, () =>
      this.db
        .exec(`CREATE VIRTUAL TABLE IF NOT EXISTS chunks_fts USING fts5(heading,text,content='chunks',content_rowid='rowid',tokenize='unicode61');
...
INSERT INTO chunks_fts(chunks_fts) VALUES('rebuild');`)
  ...
  rebuild(): Promise<KnowledgeSyncResult> {
  ...
  private runSync(force: boolean): Promise<KnowledgeSyncResult> {
    return this.write(async () => {
      ...
      const current = (
        this.db.prepare(`SELECT ${fileColumns} FROM files`).all() as any[]
      ).map((row) => mapFile(row));
      ...
      for (const file of discovery.files) {
        ...
        if (!force && old?.size === file.size && old.mtimeMs === file.mtimeMs) {
          unchanged++;
          continue;
        }
        parsed.set(
          file.path,
          await this.parseFn(file, this.config.limits.maxFileBytes)
        );
      }
      let added = 0,
        updated = 0,
      ...
      immediateTransaction(this.db, () => {
        ...
        for (const old of current)
          if (!paths.has(old.path)) {
            this.db.prepare("DELETE FROM files WHERE path=?").run(old.path);
            removed++;
          }
        for (const file of discovery.files) {
          ...
          this.db
            .prepare(
              "INSERT INTO files(path,root,relative_path,size,mtime_ms,chunk_count) VALUES(?,?,?,?,?,?) ON CONFLICT(path) DO UPDATE SET root=excluded.root,relative_path=excluded.relative_path,size=excluded.size,mtime_ms=excluded.mtime_ms,chunk_count=excluded.chunk_count"
            )
            .run(
              file.path,
              file.root,
          ...
          const insert = this.db.prepare(
            "INSERT INTO chunks(id,file_id,ordinal,heading,text,start_line,char_offset) VALUES(?,?,?,?,?,?,?)"
          );
  ...
  size(): number {
    return Number(
      (this.db.prepare("SELECT count(*) n FROM files").get() as any).n
    );
  }
  chunkCount(): number {
    return Number(
      (this.db.prepare("SELECT count(*) n FROM chunks").get() as any).n
    );
  }
  listFiles(): IndexedKnowledgeFile[] {
    const rows = this.db
      .prepare(`SELECT ${fileColumns} FROM files ORDER BY path`)
      .all() as any[];
    const headingQuery = this.db.prepare(
      "SELECT DISTINCT heading FROM chunks WHERE file_id=? AND heading<>'intro' ORDER BY ordinal"
    );
    return rows.map((row) =>
      mapFile(
        row,
        (headingQuery.all(row.id) as any[]).map((item) => String(item.heading))
      )
    );
  }
  getFile(value: string): IndexedKnowledgeFile | undefined {
    const row = this.db
      .prepare(
        `SELECT ${fileColumns} FROM files WHERE path=? OR relative_path=? ORDER BY path LIMIT 1`
      )
  ...
  listChunks(fileId: number): IndexedKnowledgeChunk[] {
    return (
      this.db
        .prepare(
          "SELECT c.*,f.path FROM chunks c JOIN files f ON f.id=c.file_id WHERE c.file_id=? ORDER BY c.ordinal"
        )
        .all(fileId) as any[]
    ).map(mapChunk);
  }
  search(query: string, limit = 10): KnowledgeSearchResult {
    ...
    const rows = this.db
      .prepare(
        "SELECT c.*,f.path,bm25(chunks_fts) rank FROM chunks_fts JOIN chunks c ON c.rowid=chunks_fts.rowid JOIN files f ON f.id=c.file_id WHERE chunks_fts MATCH ? ORDER BY rank ASC,f.path ASC,c.ordinal ASC,c.id ASC LIMIT ?"
      )
      .all(compiled, Math.min(Math.max(limit, 1), 100)) as any[];
    return {
      status: "available",
...
export const knowledgeIndexLayer = (
  path: string,
  config: KnowledgeConfig,
    ...
    sqliteResource(path, factory).pipe(
      Effect.map((db) =>
        Context.make(
          ...
          new KnowledgeIndex(db, config, options)
        )
      ),
      Effect.mapError((cause) =>
        cause instanceof ContextStorageError
          ...
          : new ContextStorageError({
              ...
              message: "Cannot initialize knowledge index database",
              cause,
            })
      )
    )
