...
test("host template renders a secure full-viewport dark shell and wires lifecycle UX", () => {
	...
	assert.match(html, /id="loading" class="fixed inset-0 flex h-dvh w-screen[^>]*aria-label="Loading App"/);
	assert.match(html, /h-dvh min-h-screen overflow-hidden/);
	assert.match(html, /id="app" class="fixed inset-0 h-dvh w-screen border-0 bg-pi-bg" hidden/);
	assert.match(html, /<header class="fixed [^"]*z-10[^"]*" aria-label="Pi App navigation">/);
	...
	assert.match(script, /displayMode: 'fullscreen'/);
	assert.match(script, /availableDisplayModes: \['fullscreen'\]/);
	assert.match(script, /containerDimensions: \{/);
	assert.match(script, /width: Math\.max\(1,/);
	assert.match(script, /height: Math\.max\(1,/);
	assert.match(script, /bridge\.setHostContext\(hostContext\(\)\)/);
	assert.match(script, /addEventListener\('resize', scheduleHostContext\)/);
	assert.match(script, /addEventListener\('orientationchange', scheduleHostContext\)/);
	assert.match(script, /visualViewport\?\.addEventListener\('resize', scheduleHostContext\)/);
	assert.match(script, /resizeFrame !== undefined/);
	assert.match(script, /requestAnimationFrame\(updateHostContext\)/);
	...
	assert.match(script, /bridge\.onrequestdisplaymode = async \(\{ mode \}\) => \{ if \(mode !== 'fullscreen'\) return \{ mode: 'fullscreen' \};/);
	assert.match(script, /post\('\.\/display-mode', \{ mode: 'fullscreen' \}\)/);
...
test("host lifecycle detaches locally on pagehide and only explicitly completes once", () => {
	...
	assert.match(teardown, /removeEventListener\('resize', scheduleHostContext\)/);
	assert.match(teardown, /removeEventListener\('orientationchange', scheduleHostContext\)/);
	assert.match(teardown, /visualViewport\?\.removeEventListener\('resize', scheduleHostContext\)/);
	assert.match(teardown, /cancelAnimationFrame\(resizeFrame\)/);
