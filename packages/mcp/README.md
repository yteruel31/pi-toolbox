...
## Optional operation authorization

Model entry points use the pure `@yteruel31/pi-operation-hooks` protocol on `pi.events`. MCP contains no policy rules or guardrails dependency. `McpRuntimeOptions.operationBus` and per-call `McpOperationContext` allow embedding with an optional bus, Pi context, and root tool-call ID. No registered provider preserves existing behavior; a registered veto, failed assessment, or cancellation prevents the described operation. Existing gateway confirmations, rollback and lifecycle checks still apply after authorization. Result observers receive only a once-only error boolean, never remote output.

Operation names are `tools-call`, `tools-discover` (unqualified-name resolution), `tools-search`, `tools-list`, `connect`, `status`, `diagnostics`, `resources-list`, `resources-read`, `prompts-list`, `prompts-get`, `auth-start`, `auth-complete`, `gateway-status`, `gateway-validate`, `gateway-configure`, `gateway-deactivate`, and `apps-open`. Direct tools and the `mcp` wrapper share one `tools-call` authorization with the resolved server, original MCP tool name and actual arguments, before even lazy connection. Unqualified names first authorize discovery, then authorize the resolved call. Discovery authorization does not authorize a tool call.

Only caller arguments and operation identity are submitted: no resolved transport environment, headers, OAuth credentials, backend secrets or capability URLs. `auth-complete` omits `redirectUrl` entirely. Tool arguments remain intact for local deterministic policy matching; the provider owns masking before displaying, auditing or sending them to an assessor. Model-triggered App loading/publication has a separate `apps-open` gate with the declared resource URI; refusal leaves the original tool result intact and marks its UI unavailable.

**Bounded scope:** these hooks protect model entry points, not every manager or HTTP request. Human `/mcp` panel actions, opted-in background direct-tool discovery, server list-change refreshes, protocol reconnect/auth mechanics, OAuth browser callbacks, sampling/elicitation and cleanup retain their existing controls. The App controller's secret-protected HTTP `tool-call` route remains browser-consented, server-scoped `callFromApp`, not a model `tools-call`; its message/context/display-mode/heartbeat/complete routes remain local session intents, and `open-link` remains validated browser delegation. These browser routes do not inherit a model approval or emit model operation hooks. The hooks are not a transport sandbox and cannot revoke already-running remote work.
