...
| UI panel / commands | `/mcp` management panel, compact live MCP footer, persistent Apps status link, and a panel-only `/mcp-gateway` setup/diagnosis/removal flow | **Supported subset:** server/tool search, enable/disable, direct-tool selection, reconnect, OAuth start, and explicit publication setup are supported; adapter import/provenance, trace, logout, and keyring UI remain deferred |
...
3. **Configure explicitly.** Run `/mcp-gateway`, choose managed Tailscale or a preconfigured custom HTTPS reverse proxy, and complete the external challenge. Nothing is persisted until the exact capability path validates end to end.
