...
| MCP Apps / remote access | Official AppBridge, isolated loopback host, and explicit managed-Tailscale or custom-HTTPS capability gateway | **Supported subset, design difference:** custom proxy configuration/access policy remains user-owned and no UI opens automatically |
...
| UI panel / commands | Unified `/mcp` Servers/Gateway/Diagnostics modal, compact live footer, Apps status link, validated gateway tool actions, and explicit current-agent setup/repair prompts | **Supported subset:** server/tool search, enable/disable, direct-tool selection, reconnect, OAuth start, and explicit publication setup are supported; adapter import/provenance, trace, logout, and keyring UI remain deferred |
...
3. **Configure explicitly.** Open `/mcp` > Gateway, choose managed Tailscale or Custom's agent-assisted proxy setup, confirm the changes, and complete the external challenge. Nothing is persisted until the exact capability path validates end to end.
...
6. **Rollback on any discrepancy.** Open `/mcp` > Gateway and deactivate publication, restore the previous Pi extension selection, and continue with the untouched adapter/configuration. Managed Tailscale removal affects only the exact route; custom proxy infrastructure must be removed separately by its owner. Stored pi-mcp OAuth/cache files may be removed separately after provider-side revocation where appropriate.
