...
Use `/subagents`, then Setup, to stage the user-global opt-in and credential source. Nothing, including a key, is written until Save; Cancel discards the draft. API-key input is masked, and Test connection is an explicit action. Supported sources are a syntactically valid environment-variable reference (up to 256 characters; `JEV_API_KEY` and `TYPESAFE_API_KEY` are recommended), Linux Secret Service through `secret-tool`, or an explicitly selected private mode-0600 file outside repositories. The dedicated keyring namespace is `application=pi-subagents, service=jev`; keys are passed over stdin, never argv. There's no automatic fallback from keyring to a plaintext file.
