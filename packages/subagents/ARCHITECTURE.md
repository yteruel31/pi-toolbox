...
The Claude adapter lazily loads `@anthropic-ai/claude-agent-sdk`, bridges cancellation to a per-call controller, and starts `query()` with a bounded producer implementing `AsyncIterable<SDKUserMessage>`. Accepted continuation messages enter that same producer; result accounting keeps the query alive only while accepted turns remain. The adapter consumes the query iterator once, closes it once, captures assistant/tool input/progress/result records, and maps the latest SDK result/model usage into the shared result shape. It uses the stock Claude Code system prompt plus the selected named-agent prompt. Model-catalogue discovery uses a separate message-free temporary query, calls `supportedModels()`, and bounds its lifetime with abort, timeout, and unconditional close cleanup.

Headless execution uses `bypassPermissions` and `allowDangerouslySkipPermissions`. `settingSources: []` and `persistSession: false` prevent user/project Claude settings, hooks, `CLAUDE.md`, MCP configuration, and persisted child transcripts from altering the run. No `.claude` routing or additional configuration fields are imported. Missing SDK, executable, auth, incompatible SDK, result, and iterator failures become bounded typed diagnostics.

The shared resolved `thinking` value is adapted at the SDK boundary: `off` emits disabled thinking, `minimal` emits effort `low`, and the other levels map one-to-one. The adapter does not infer per-model effort availability, clamp a value, or retry with a different value; the SDK remains authoritative and a rejection settles the run as a bounded failure.
...
Route precedence is field-by-field for harness, model, and thinking:

1. explicit spawn arguments;
2. trusted project mapping;
3. user mapping;
4. named-agent frontmatter;
5. parent defaults (Pi harness only).

For the Claude harness, profile `effort` is the profile-level source for resolved thinking ahead of profile `thinking`; saved or explicit `thinking` still wins because it occupies a higher layer. `effort` alone never infers the harness. The public spawn override remains `reasoning_effort`, and persisted routing plus the UI expose only `thinking`, not a second effort field. A harness-only saved route can therefore select Claude while model and effort continue to come from profile defaults.
...
Jev is an opt-in user-global layer that runs after ordinary per-field precedence and before the selected child backend starts. Spawn returns its run id before model discovery, credential resolution, or the routing SDK executes. It only fills unconstrained fields; explicit spawn values, trusted project or user mappings, and agent defaults remain fixed. Candidate construction uses Pi's current scoped models, or all available models when unscoped, with runtime thinking metadata, and Claude SDK `supportedModels()` metadata. A fixed available model may be added outside scope, but an inherited parent model is never treated as fixed. A fixed backend filters incompatible candidates instead of being rewritten. Tool compatibility rejects the other backend's native naming dialect, not unknown extension names: a profile allowlist restricts exposure and is not a tool-availability requirement. The original allowlist reaches the harness unchanged, where Pi intersects it with active tools and safety exclusions. No parent extension is enabled by routing. Empty candidate sets report categorical catalogue/scope, tool, model-match, or effort diagnostics without echoing arbitrary input strings; the background harness records a persisted `failed` routing state before propagating the conflict.

One bounded SDK choice carries the task, role, candidate descriptions, and explicit constraints. It carries no parent transcript or extra file content. Catalogues above the service's 255-choice limit aren't truncated and instead use the ordinary route. Unknown model capabilities are represented only by runtime facts. Timeout, transport, oversized response, malformed answer, and unavailable-choice failures make one visible categorical fallback to the ordinary route, with no retry. The bounded transcript exposes pending state followed by the effective backend/model/thinking and categorical per-field provenance; it does not expose prompts, keys, or raw provider errors. The integration does not perform live route-quality evaluation.

Jev credentials use a dedicated `pi-subagents` namespace. Setup stages the opt-in and credential choice until Save, supports bounded syntactically valid environment references, Linux Secret Service, or an explicitly chosen private file, and never silently falls back to weaker storage. Opt-in changes become active only after reload.
...
`/subagents agents` opens a fresh full-terminal routing overlay with user/project scope, edit/delete, trust gating, and explicit invalid-file backup/reset. The route field editor is an internal panel state rather than a nested `ctx.ui.select`/`ctx.ui.editor` flow, so Enter transitions within the same overlay and Escape returns to the mapping list. Harness, model, and thinking are finite left/right selectors; thinking is the sole Pi-thinking/Claude-effort control. Pi choices come from the active scoped models, falling back to the available registry when unscoped; a scoped model's pinned thinking level updates the thinking selector when chosen. Claude choices come from SDK `supportedModels()`, which does not guarantee that each effort level is effective for every returned model. Catalogue values are deduplicated, saved aliases retain their original spelling, and absent saved values remain explicit legacy choices. Overlays are not reused after close.
