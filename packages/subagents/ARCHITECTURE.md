...
`/subagents agents` opens a fresh full-terminal routing overlay with user/project scope, edit/delete, trust gating, and explicit invalid-file backup/reset. The route field editor is an internal panel state rather than a nested `ctx.ui.select`/`ctx.ui.editor` flow, so Enter transitions within the same overlay and Escape returns to the mapping list. Harness, model, and thinking are finite left/right selectors. Pi choices come from the active scoped models, falling back to the available registry when unscoped; a scoped model's pinned thinking level updates the thinking selector when chosen. Claude choices come from SDK `supportedModels()`. Catalogue values are deduplicated, saved aliases retain their original spelling, and absent saved values remain explicit legacy choices. Overlays are not reused after close.
