...
## Operation authorization (optional consumer)

Main-session tools describe operations through the pure `@yteruel31/pi-operation-hooks` **0.1.0** dependency on `pi.events`. This package contains no policy rules, guardrails imports, judge, or approval UI. A consumer registers synchronously on the shared bus; registered gates are awaited before the described action. A denial, broken registered gate, or cancelled authorization prevents that action. Without a provider, behavior is unchanged. This adds no worker tools and does not activate any extension or change manual `/web-access` setup/diagnostic actions.

| Operation name | Authorized scope |
| --- | --- |
| `web_search`, `source_check` | All supplied queries to the selected/configured search provider, before credential resolution; provider API destination, not a claim that upstream page domains are controlled |
| `fetch_content` | All batch destinations together, with local video paths represented as canonical absolute `file:` URLs as well as resolved paths (an HTTP-domain allowance cannot cover them); extraction, cache writes, optional public GitHub clone, media processing and rendering. Search/includeContent and source-check authorize a separate discovered-source batch before fetching any of it |
| `fetch_content.request` | New direct-HTTP redirect and rendered page/subresource destinations, before DNS/connection; also YouTube metadata CONNECT destination origins |
| `fetch_content.media` | Resolved YouTube download **origin only**, before fetching; signed URL credentials are deliberately excluded |
| `synthesis` | Selected Pi provider/model receiving retrieved content; Pi manages the actual transport/authentication |
| `get_search_content` | Local stored-content read, including saved research reports |
| `deep_research.start` | Provider/model/subject and resolved output path, including background submission, lifecycle polling, tracking and eventual publication |
| `deep_research.status` | Local list or saved job inspection; a job status can retrieve from the provider, attach a recovered upstream ID, publish a report and authorize continuing lifecycle polling |
| `deep_research.result` | Saved provider retrieval and publication, including a replacement resolved output path applied under the job lock before refresh can publish; its internal refresh is not evaluated again |
| `deep_research.cancel` | Explicit native cancellation using the saved provider; no resolved keys, subject, report or private trajectories in cancellation metadata |
| `deep_research.resume` | Restart recovery of a saved job's polling/publication scope, using a noninteractive context; denied jobs remain tracked and await an explicit status call |
| `reddit_search`, `reddit_fetch_content` | Exact canonical request URL and original arguments before configuration reads, profile locking/browser launch and cache writes; diagnostic-selected posts get a separate internal fetch gate |
| `reddit_profile_diagnostic.inspect`, `.test` | Local-only readiness inspection versus a real authenticated search/post test |
| `web_access_diagnostic.inspect`, `.test_render` | Local prerequisites versus a synthetic isolated browser launch; neither makes external requests |

Each operation retains the root Pi tool-call ID and tool name. The initial direct fetch is covered by its containing batch approval, not evaluated again at the transport layer. New destinations and synthesis are separate suboperations. Receipts report the returned action's success/error exactly once, without result bodies, including resolved diagnostic failures and research error/output-error/failure states. A start receipt reports scheduling, **not** eventual upstream research success. Polling after an authorized start/status does not open further approval dialogs; it runs within that job's approved lifecycle. Restart authorization uses a context with no UI, so a policy requiring confirmation must refuse it and wait for an explicit tool call. No-provider restart polling remains automatic. Stopping Pi stops local polling, not the upstream job.

Registered tools deep-snapshot validated arguments before updates or asynchronous authorization; metadata and execution use that same private snapshot. Direct research start inputs are also snapshotted. Absent optional metadata is omitted, without JSON-normalizing unknown objects. Metadata is constructed from these arguments and operation-owned provider/path information before resolving credentials; resolved API keys, profile data, headers, signed media queries and provider responses are never authorization inputs. Consumers must mask potentially sensitive original arguments before any judge/history/UI use. Local record reads needed to identify a research job occur before its provider gate, but credential resolution, outbound calls and publication occur after approval. Cancellation while approval is pending performs none of those actions.

**Limits:** these are cooperative main-session operation hooks, not a process/network sandbox. Search and native research providers choose and fetch upstream pages opaquely: allowing their API endpoint cannot enforce a page-domain allowlist inside those providers. Pi synthesis transport endpoints are also provider-owned. Public GitHub clones are approved at the repository URL, with the existing pinned-GitHub/no-redirect restrictions. YouTube tunnels are gated by destination origin, not encrypted paths; resolved media URLs are similarly origin-only to avoid leaking signed credentials. The existing SSRF, browser, Git and media restrictions still apply independently. Direct HTTP redirects and the isolated general renderer's parent-routed HTTP requests are gate-visible. Native Reddit browser background traffic remains subject to its existing application restrictions, not a new general interception sandbox. Existing approval scopes are not retroactively revoked when policy changes; restart recovery or a new explicit tool call is evaluated again.
