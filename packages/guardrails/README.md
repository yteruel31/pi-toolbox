...
Unsupported commands/options, interpreters, executable scripts, wrappers, cwd changes, expansions (including substitutions inside double quotes), globs, heredocs and syntax outside the bounded grammar remain unresolved. They proceed to model assessment when Judge is On and use rule-only no-match behavior when Judge is Off; they are never given an unconditional built-in Ask or Allow. An ordinary CLI's quoted prompt isn't parsed as shell code. This does not add autoapproval presets or attempt full shell emulation.
