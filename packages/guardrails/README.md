...
With model judgment On, an explicit structured Allow for shell calls must match an exact, simple command. Complex syntax, wrappers or interpreters can't use this shortcut, even if the full command string matches. In either judge mode, a deterministic shell Ask or Deny cannot be overridden by an exact Allow, natural policy or model verdict. File Allows can use a component-bounded path prefix. With the judge On, natural-language restrictions matching the actor/tool/conditions are assessed before a structured Allow applies.

### Rule-only mode (Judge model Off)

This is deliberately permissive: only deterministic rules run, with **Deny > Ask > Allow** precedence. **No matching deterministic rule means Allow, on both main and Pi workers.** Applicable built-in argument-budget and self-protection checks still run for enabled modules. Structured Allows apply using their declared conditions, including complex shell calls; they never override Ask/Deny. Worker Ask still blocks without a popup, while main Ask retains its approval choices.
