...
Setup can select the default Pi backend or the optional Jev backend. Jev uses a Choice question plus one policy-specific probability question for every applicable natural restriction. Its default automatic thresholds are 0.95 for Allow and 0.80 for Deny; intermediate or conflicting evidence asks. These probabilities are model estimates, not calibrated guarantees. A natural Allow never overrides a restriction. Any SDK, credential, timeout, cancellation, malformed-response, or size failure closes to Ask/Deny according to the existing error rules, with no fallback backend and no retry.
...
Jev credentials are configured independently in Setup. Environment mode stores only the variable name and reads its value at assessment time. Keyring mode uses the fixed `pi-guardrails/jev` identity and requires `libsecret-tools` (`secret-tool`), a session D-Bus, and an unlocked Secret Service collection; failure never falls back to a file. File mode requires an absolute owner-private path outside a repository, an owner-private directory, and a regular 0600 file. Setup's reference and masked-key editors stage values in memory: Enter stages, Escape cancels, and only the main Save confirmation persists configuration or a key. Closing/cancelling writes nothing. Keys are never stored in guardrails configuration, history, logs, model inputs, or the Pi transcript by this package.
...
Old version 1 configs remain valid: omitted `coverage` switches and `judgeEnabled` default to true, preserving existing behavior when protection is enabled. New installations default to global Protection Off and thinking off, and omit the broad destructive-files Ask policy so proven-safe scoped cleanup can stay fluent. Existing saved policies are loaded exactly as configured; no migration weakens an Ask or Deny. Project config cannot set these switches or weaken global settings.

Start with the built-in presets in the UI: destructive Git, destructive files, system changes, production operations and secrets. The destructive-files preset remains available for opt-in or existing configurations. The minimal example above doesn't implicitly include omitted presets.
...
Tests use fake completions, injected SDK fetches and Secret Service adapters, temporary configuration/storage, native Pi themes and resource-loader fixtures. No dangerous commands, live keyring access, credential provisioning, network requests, or live provider calls are executed. Native TUI snapshots cover dark/light themes, wide/narrow/small terminals, full-height framing, icon colors, selection stability and keyboard handling.
