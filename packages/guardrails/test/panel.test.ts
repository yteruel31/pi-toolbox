...
test("small Setup keeps the selected setting visible instead of letting help consume the viewport", () => {
  const f = fixture(nativeTheme("light"), 16, { ...initialPanelState(), setupIndex: 8 });
  try {
    ...
    f.panel.handleInput("\x1b"); assert.equal(f.state.setupIndex, 8);
  } finally { f.panel.dispose(); f.history.close(); }
});
...
test("Setup native model/thinking submenus retain focus, stage changes and never touch parent settings", () => {
  for (const name of ["dark", "light"]) {
    ...
    setup.handleInput("\x1b"); assert.equal(state.setupIndex, 8); assert.equal(draft.model, "missing/saved");
    setup.handleInput("\r"); setup.handleInput("plain"); setup.handleInput("\r");
    assert.equal(draft.model, "fake/plain"); assert.equal(state.setupIndex, 8); assert.equal(draft.thinking, "high");
    setup.handleInput("\x1b[B"); assert.equal(state.setupIndex, 9);
...
test("judge-off Setup never accesses catalog; model/thinking stay inactive and values survive re-enable", () => {
  ...
  setup.handleInput("\x1b[A"); setup.handleInput("\x1b[A"); setup.handleInput("\r");
...
test("m is no longer a panel action; submenu cancel and tab preserve Setup row", () => {
  ...
  try {
    ...
    for (let i = 0; i < 8; i++) f.panel.handleInput("\x1b[B");
    f.panel.handleInput("\r"); f.panel.handleInput("\x1b");
    assert.equal(f.done(), ""); assert.equal(f.state.setupIndex, 8);
    f.panel.handleInput("\r"); f.panel.handleInput("\t"); assert.equal(f.state.tab, "Policies");
    f.panel.handleInput("\x1b[Z"); assert.equal(f.state.tab, "Setup"); assert.equal(f.state.setupIndex, 8);
    f.draft.judgeEnabled = false;
