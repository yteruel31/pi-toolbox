...
const entrySchema = z.object({
  ...
  failure: z.enum(["credentials", "timeout", "transport", "invalid-response", "cancelled", "model-selection"]).optional(),
  jev: z.object({
    ...
    risks: z.array(z.tuple([z.enum(["sensitive-transfer", "external-modification", "unrecoverable-loss", "guardrail-modification", "uncertainty"]), z.number().min(0).max(1)])).max(5).optional(),
    thresholds: z.object({ allow: z.number().min(0).max(1), deny: z.number().min(0).max(1), restrictive: z.number().min(0).max(1) }).strict(),
    reasons: z.array(z.enum(["generic-allow", "generic-deny", "restrictive-policy", "generic-uncertain", "incomplete-input", "redacted-input", "atomic-risk"])).max(7),
  }).strict().optional(),
